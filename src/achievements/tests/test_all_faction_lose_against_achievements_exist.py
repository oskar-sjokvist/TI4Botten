...
def test_all_factions_have_lose_against_achievement():
    ...
    for p in achievements_dir.glob("*.json"):
        ...
        faction_name = filt.get("lose_against")
        if faction_name:
    ...
    assert not missing, f"Missing lose_against achievements for factions: {sorted(missing)}"
