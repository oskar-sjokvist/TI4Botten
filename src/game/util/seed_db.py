...
def main():
    ...
    with open(here / "player.csv", mode="r", newline="", encoding="utf-8") as file:
        reader = csv.reader(file)
        for row in reader:
            name_to_player_id[row[1]] = row[0]
        ...
        for item in items:
            ...
            if not game:
                ...
                game = model.Game(
                    game_id=game_id,
                    name=game_name,
                    game_state=model.GameState.FINISHED,
                    game_finish_time=datetime.fromordinal(
                        len(unique_games) + 1 - game_id
                    ),
                )
                session.add(game)
            player = session.merge(
                model.Player(player_id=name_to_player_id[player], name=player)
            )
            session.flush()
            session.merge(
                model.GamePlayer(
